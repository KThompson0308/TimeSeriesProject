...
Our Objective is to predict the stock price of Apple both individually and in conjunction with other financial and economic indicators. We believe that any business with an interest in financial markets would find a case study like this useful.  
...
We collected the data for a business cycle - 03/17/2015 to 03/19/2021
...
```{r Datacleaning, echo=FALSE, message=FALSE, warning=FALSE}
### APPL - Date & Adj.Close
head(APPL)
...
```{r Creating data by merging parameters, echo=FALSE, message=FALSE}
...
summary(Appl.data)
```
What follows are correlation plots to understand how Apple stock is related to other prices. NASDAQ, Gold and CQQQ are highly correlated. Copper and TNYR are moderately correlated. And CAC40, DXY and WTI have a low correlation with Apple stock. 
...
## UNIVARIATE APPLE STOCK PRICE MODELS

Checking the stationarity of the series and running the decomposition to see the trend and the seasonality

```{r Checking-Stationarity and decomposing, echo=FALSE, message=FALSE, warning=FALSE, results="hide"}
...
We are now ready to begin the process of identification in earnest. We first take the first difference to stationarize the series and then verify with overfitting model to see any other factor that may influence the stability of the series. We then use the ACF and PCF of this differenced series to gain initial evidence on what type of model we may be looking at. We do not note any behavior that particularly stands out, so we move onto using information criteria as a means of identifying the stationary component of our models.

```{r first-difference Overfitting, echo=FALSE, warning=FALSE, message=FALSE, results="hide"}
...
Our results indicate that we should fit either an ARIMA(4,1,2) or an ARIMA(5,1,2)
...
Appl.fore.7days = fore.aruma.wge(Appl.stock, phi = Appl.stock.est1$phi, theta = Appl.stock.est1$theta,  d=1, n.ahead = 7,limits = T, lastn = F, plot=FALSE)
Appl.fore.7days$f
#plot next 7 days
...
Estimating the model 2 (ARIMA(5,1,2)); finding ASE and Rolling ASE for short-term (7 days) and long-term (30 days) prediction; doing the short-term and long-term forecasts

```{r Model2-ARIMA512, echo=FALSE}

Appl.stock.est1 = est.arma.wge(Appl.stock.1, p=5, q=2) # burg and mle are same
#Appl.stock.est1$phi
...
{
  forecasts = fore.aruma.wge(Appl.stock[i:(i+(trainingSize-1))],phi = phis,
                             theta = thetas, d=1, n.ahead = horizon, plot=FALSE)
...
We first construct the deterministic component of our model and then treat the residuals as a
stationary random process. We make use of lung-box test to see if the residuals are whitened by the deterministic component and then we move on to modeling the residuals after finding no evidence to suggest that they have been whitened.

```{r Signal-plus-model, echo=FALSE, results='hide'}
...
##=============Short term (7 days) model =======================###
## short term (7 days) ASE 
det.appl.fcst.last.7 = fore.sigplusnoise.wge(Appl.stock[1:1488], max.p = 2, n.ahead = 7, limits = F)
#det.appl.fcst.last.7$f

ASE_Sigplus.7 = mean((Appl.stock[1488:1494] - det.appl.fcst.last.7$f)^2)
#ASE_Sigplus.7
...
## Long term (30 days) ASE 
det.appl.fcst.last.30 = fore.sigplusnoise.wge(Appl.stock[1:1464], max.p = 2, n.ahead = 30, limits = F)
#det.appl.fcst.last.30$f

ASE_Sigplus.30 = mean((Appl.stock[1465:1494] - det.appl.fcst.last.30$f)^2)
#ASE_Sigplus.30
...
```
### NEURAL NET UNIVARIATE MODEL

We use the nnfor API and the mlp function for the univariate case.

```{r Univeriate-nn-model, echo=FALSE, results='hide'}
...
mlp1 = readRDS("univariatemlpfit1.rds")
mlp2 = readRDS("univariatemlpfit2.rds")

#appl.uni.fit.mlp = mlp(appl.uni.trn.7,reps = 50,comb = "mean")
#appl.uni.fit.mlp

appl.uni.fore.mlp = forecast(mlp1, h = 7)
...
fore.mlp.APPL = forecast(mlp1, h=7)
tp.APPL=data.frame(fore.mlp.APPL)
APPL.7 = tp.APPL$Point.Forecast

plot(seq(1,100,1),APPL[1395:1494], type = 'l', xlim = c(1,115), main="MLP 7 day forecast")
...
appl.uni.fore.mlp.2 = forecast(mlp2, h = 30)
plot(seq(1,100,1),APPL[1365:1464], type = 'l', xlim = c(1,115), main ="MLP 30 day forecast")
lines(seq(101,130,1), appl.uni.fore.mlp.2$mean, col = 'blue')
## ASE for 30 days
ASE.appl.30 = mean((APPL[1465:1494] - appl.uni.fore.mlp.2$mean)^2)
#ASE.appl.30
# 30days forecast
fore.mlp.APPL = forecast(mlp1, h=30)
tp.APPL=data.frame(fore.mlp.APPL)
...
### ARIMA + NN  &  SignalPlus and Neural Net 

```{r Ensemble-Univeriate, echo=FALSE, results='hide'}
## Building Ensemble model using ARIMA- model2 and Neural Net
## ===>>>>>>>>>> Short term (7 days) model using >>>>>>>>>>======
ensemble_ARIMA_nn.7 = (Appl.fore.lastn.7$f + appl.uni.fore.mlp$mean)/2
ASE_ARIMA_nn.7 = mean((Appl.data$APPL[1488:1494] - ensemble_ARIMA_nn.7)^2)
#ASE_ARIMA_nn.7
## forecasting for 7 days
ensemble_fore_ARIMA_nn.7  = (Appl.fore.7days$f + APPL.7)/2
#Plot for 7 days forecasting
plot(seq(1,100,1), Appl.stock[1395:1494], type = "l",xlim = c(0,115), 
     ylab = "Stock Price", main = "7 Days Forecast")
lines(seq(101,107,1), ensemble_fore_ARIMA_nn.7, col = 'blue')

## ===>>>>>>>>>> Short term (7 days) model using >>>>>>>>>>======
ensemble_ARIMA_nn.30 = (Appl.fore.lastn.30$f + appl.uni.fore.mlp.2$mean)/2

ASE_ARIMA_nn.30 = mean((Appl.data$APPL[1465:1494] - ensemble_ARIMA_nn.30)^2)
#ASE_ARIMA_nn.30
## forecasting for 30 days
ensemble_fore_ARIMA_nn.30  = (Appl.fore.30days$f + APPL.30)/2
#Plot for 7 days forecasting
plot(seq(1,100,1),Appl.stock[1395:1494], type = 'l', xlim = c(1,140))
lines(seq(101,130,1), ensemble_fore_ARIMA_nn.30, col = 'blue')

## Building Ensemble model using SignalPlus and Neural Net
## ===>>>>>>>>>> Short term (7 days) model using >>>>>>>>>>======
ensemble_Sigplus_nn.7 = (det.appl.fcst.last.7$f + appl.uni.fore.mlp$mean)/2

ASE_Sigplus_nn.7 = mean((Appl.data$APPL[1488:1494] - ensemble_Sigplus_nn.7)^2)
ASE_Sigplus_nn.7
## forecasting for 7 days
ensemble_fore_Sigplus_nn.7  = (det.appl.fcst.7$f + APPL.7)/2
#Plot for 7 days forecasting
plot(seq(1,100,1), Appl.stock[1395:1494], type = "l",xlim = c(0,115), 
     ylab = "Stock Price", main = "7 Days Forecast")
lines(seq(101,107,1), ensemble_fore_Sigplus_nn.7, col = 'blue')

## ===>>>>>>>>>> Long term (30 days) model using >>>>>>>>>>======
ensemble_Sigplus_nn.30 = (det.appl.fcst.last.30$f + appl.uni.fore.mlp.2$mean)/2

ASE_Sigplus_nn.30 = mean((Appl.data$APPL[1465:1494] - ensemble_Sigplus_nn.30)^2)
#ASE_Sigplus_nn.30
## forecasting for 30 days
ensemble_fore_Sigplus_nn.30  = (det.appl.fcst.30$f + APPL.30)/2
#Plot for 7 days forecasting
plot(seq(1,100,1),Appl.stock[1395:1494], type = 'l', xlim = c(1,140))
lines(seq(101,130,1), ensemble_fore_Sigplus_nn.30, col = 'blue')
```
## MULTIVARIATE SECTION 

### VAR AND MULTIVARIATE ARIMA MODELS
...
```{r aic5, echo=FALSE, warning=FALSE, message=FALSE, fig.show='hide'}
basemod = lm(X$Adj.Close ~ X$IXIC + X$FVX + X$IRX)
summary(basemod)
aic5.wge(basemod$residuals)
```

We can see from the AIC5 table that our preferred model for the residuals is ARIMA(4,1,2).
...
for( i in 1:(len-(trainingSize + horizon) + 1))
{
  ...
  fit2 = arima(data[i:(i+(trainingSize-1)),3], order=c(4,1,2),xreg = data[i:(i+(trainingSize-1)),-3])
...
The multivariate ARIMA(2,1,0) model seems to have the absolute best performance.
...
```{r Multivariate-Model-Using-nn, echo=FALSE, results='hide'}
...
# Load Saved Neural Nets File
nets = readRDS("multivarNNmodels.rds")
...
out.test.7 = compute(nets$net7, test.Appl.data.7[,2:9]) # prediction
...
out.test.30 = compute(nets$net30, test.Appl.data.30[,2:9])
...
fore.mlp.CAC40 = forecast(nets$cac, h=7)
...
out.fore.7 = compute(nets$fore7, test.appl.7days.fore[,2:9])
...
out.fore.30 = compute(nets$fore30, test.appl.30days.fore[,2:9])
...
library(knitr)
windowedases = readRDS("windowedASE.rds")
windowed7 = rbind(arima412 = windowedases$model10,
                 VAR=windowedases$var7, MultiArima = windowedases$ar7,
                 ArimaVarEnsemble = windowedases$multivar7ens, NN=ASE.nn.7)
windowed30 = rbind(windowedases$model12, windowedases$var30,
                   windowedases$ar30,windowedases$multivar30ens,
                   ASE.nn.30)
windows = cbind(windowed7,windowed30)
kable(windows, caption="Windowed ASEs",col.names = c("7 days", "30 days"))
```